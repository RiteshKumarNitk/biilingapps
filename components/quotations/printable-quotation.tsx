...
export function PrintableQuotation({ quotation, items, tenant }: PrintableQuotationProps) {
    ...
    // For quotation, discount might be column or header level? Schema says 'discount_amount' on header but items might have it too?
    // Let's assume schema header 'discount_amount' IS the total discount.
    // Or if items have it.
    // Previous code in Invoice calculated it from items. Quotation page shows 'discount_amount' from header.
    // I will use header 'discount_amount' if > 0, else aggregate items if they have meaningful discount?
    // Let's fallback to header amount for now if items don't have it explicitly.
    // Actually, usually headers are summary.
    ...
    return (
        <div className="min-h-screen bg-slate-100 print:bg-white pb-10 print:pb-0">
                ...
                <div className="max-w-[210mm] mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Button onClick={handlePrint} className="bg-blue-600 hover:bg-blue-700 text-white shadow-md">
                            <Printer className="w-4 h-4 mr-2" /> Print {quotation.type === 'proforma' ? 'Proforma' : 'Estimate'}
                        </Button>
                    </div>
                </div>
            </div>
            ...
            <div className={cn(
                ...
                <div id="print-content" className={containerClasses}>
                    <style jsx global>{`
                        @media print {
                            @page {
                                size: A4;
                    ...
                    <div className={cn("mb-6 relative", theme === 'minimal' ? "border-b pb-4" : "")}>
                        ...
                        <div className="text-center">
                            <h2 className={cn("text-xl font-bold uppercase tracking-widest decoration-slate-400 underline-offset-4", currentTheme.colors.accent, theme === 'classic' ? "underline" : "")}>
                                {quotation.type === 'proforma' ? 'Proforma Invoice' : 'Estimate / Quotation'}
                            </h2>
                        </div>
                    </div>
                    ...
                    <div className={cn("flex flex-col md:flex-row gap-0 mb-6", boxBorder)}>
                        <div className={cn("flex-1 p-4 border-b md:border-b-0 md:border-r", currentTheme.colors.border)}>
                            ...
                            <p className="font-bold text-base mb-1">{quotation.party_name}</p>
                            {/* Address would be fetched if joined, for now plain display if available in quotation fields. 
                                Quotation schema has party_id and party_name, but maybe not address snapshotted?
                                If not snapshotted, we can't show it unless we fetch party details. 
                                'getQuotation' action didn't join party details.
                                It's fine, we show what we have. 
                            */}
                            {/* If we want address, we should fetch party details in getQuotation if needed. 
                                Leaving simple for now or showing note. */}
                        </div>

                        <div className="flex-1 p-4 grid grid-cols-2 gap-y-3 gap-x-4">
                            <div>
                                <p className="text-xs text-slate-500 uppercase">Ref No.</p>
                                <p className="font-bold">{quotation.quotation_number}</p>
                            </div>
                            <div>
                                <p className="text-xs text-slate-500 uppercase">Date</p>
                                <p className="font-bold">{format(new Date(quotation.date), 'dd/MM/yyyy')}</p>
                            </div>
                            <div>
                                <p className="text-xs text-slate-500 uppercase">Valid Until</p>
                                <p className="font-bold">{quotation.valid_until ? format(new Date(quotation.valid_until), 'dd/MM/yyyy') : '-'}</p>
                            </div>
                            <div>
                                <p className="text-xs text-slate-500 uppercase">Status</p>
                                <span className={cn(
                                    "font-bold text-xs px-2 py-0.5 rounded border inline-block",
                                    quotation.status === 'converted' ? "border-green-600 text-green-700 bg-green-50" : "border-orange-200 text-orange-700 bg-orange-50"
                                )}>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className={tableHeaderClass}>
                                    ...
                                    {/* Discount col if needed per item */}
                    ...
                    <div className="mt-auto pt-6 border-t border-slate-300">
                        <div className="flex flex-col md:flex-row justify-between items-end gap-8">
                            <div className="flex-1">
                                ...
                                {quotation.notes && (
                                    <>
                                        <p className="text-xs font-bold uppercase text-slate-500 mb-2">Notes & Terms</p>
                                        <p className="text-xs text-slate-700 whitespace-pre-line">{quotation.notes}</p>
                                    </>
                                )}
                            </div>
